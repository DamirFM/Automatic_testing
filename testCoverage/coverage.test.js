...
//Тут вам поможет отчет по покрытию тестами. Работу в этом упражнении можно выстроить следующим образом:
...
//Функция get(coll, index, defaultValue = null) извлекает из массива значение по указанному индексу,
// если индекс существует. Если индекс не существует, возвращает значение по умолчанию.
...
//Функция indexOf(coll, value, fromIndex) возвращает первый индекс,
// по которому переданное значение может быть найдено в массиве или -1, 
//если такого значения нет. Аргументы:

//coll - массив, в котором ведется поиск.
//value - значение, поиск которого ведется в массиве .
//fromIndex - индекс, с которого начинается поиск элемента, по умолчанию равен нулю. 
//Если значение fromIndex отрицательное, то оно используется, как смещение с конца массива.
//Пример работы функции:
...
//Функция slice(coll, begin, end) возвращает новый массив, содержащий копию части исходного массива. 
//Аргументы:

//coll - исходный массив.
//begin - индекс, по которому начинается извлечение. Если индекс отрицательный, 
//begin указывает смещение от конца последовательности. По умолчанию равен нулю.
//end - индекс, по которому заканчивается извлечение 
//(не включая элемент с индексом end). Если индекс отрицательный, end указывает смещение от конца последовательности. По умолчанию равен длине исходного массива.
//Пример работы функции:
...
test('testing function slice()', () => {
  const actual4 = slice([1, 2, 3, 4, 5, 6], 1, 4);
  expect(actual4).toEqual([2, 3, 4]);

  // BEGIN (write your solution here)
  const actual5 = slice([1, 2, 3, 4, 5, 6]);
  expect(actual5).toEqual([1, 2, 3, 4, 5, 6]);

  const actual6 = slice([1, 2, 3, 4, 5, 6], -4, -2);
  expect(actual6).toEqual([3, 4]);

  const actual7 = slice([1, 2, 3, 4, 5, 6], 7);
  expect(actual7).toEqual([]);

  const actual8 = slice([1, 2, 3, 4, 5], -8, 8);
  expect(actual8).toEqual([1, 2, 3, 4, 5]);

  const actual88 = slice([]);
  expect(actual88).toEqual([]);
  // END
});

test('testing function indexOf()', () => {
  const actual9 = indexOf([2, 7, 3, 2, 4], 2);
  expect(actual9).toBe(0);

  // BEGIN (write your solution here)
  const actual10 = indexOf([1, 2, 3, 2, 5], 2, -3);
  expect(actual10).toBe(3);

  const actual11 = indexOf([1, 2, 3, 2, 5], 7);
  expect(actual11).toBe(-1);

  const actual12 = indexOf([1, 2, 3, 2, 5], 2, -10);
  expect(actual12).toBe(1);

  const actual13 = indexOf([1, 2, 3, 2, 5], 2);
  expect(actual13).toBe(1);
  
  const actual14 = indexOf([]);
  expect(actual14).toBe(-1);
  // END
});
